class IteratorClass:
    ...
    def __init__(self,x,y,operator):
        ...
        self.max=len(self.y)
        self.res=[]
        if((type(x) in [list,tuple]) and (type(y) in [list,tuple]) and operator in ["add","div","mul","sub"]) :
          
            if operator=="div":
                for i, j in zip(self.x,self.y):
                    self.res.append(round(i/j,2))
...
class ListV2:
    ...
    def __add__(self, lst1):
        result=[]
        if type(lst1) in [int,float]:
            for i in self.z:
                result.append(i+lst1)
        else:
            for i,j in zip(self.z,lst1.z):
                result.append(i+j)
        return ListV2(result)
    def __sub__(self, lst1):
        result=[]
        if type(lst1) in [int,float]:
            for i in self.z:
                result.append(i-lst1)
    ...
    def __mul__(self, lst1):
        ...
        if type(lst1) in [int,float]:
            ...
            for i,j in zip(self.z,lst1.z):
                result.append(i*j)
        return ListV2(result)
    def __truediv__(self, lst1):
        result=[]
        if type(lst1) in [int,float]:
            for i in self.z:
                result.append(round(i/lst1,2))
        else:
            for i,j in zip(self.z,lst1.z):
                result.append(round(i/j,2))
        return ListV2(result)
    ...
    def __next__(self):
        if(self.num >= len(self.z)):
            raise StopIteration
        result = self.z[self.num]
        self.num += 1
        return result
